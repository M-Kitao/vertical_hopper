...
CoT⁻¹ (Energy Efficiency) の比較グラフを生成するスクリプト。

- baseline は default と Gain-only の 2 つのみを対象（Torque は除外）
- evaluate_robustness.py の plot_results と同等の実装方針に統一

使用方法:
  # デフォルト（results/robustness_eval 以下の JSON を自動検出）
  python cot_inv.py --input-dir results/robustness_eval
...
# プロット
# =============================================================================
def plot_cot_inv_results(all_results: dict, output_dir: str) -> None:
    """
    CoT⁻¹ の棒グラフを生成して保存する。
    ...
    metric = "mean_cot_inv"
    std_key = "std_cot_inv"        # evaluate_robustness.py の格納キーに合わせる
    title = "CoT⁻¹ (Efficiency)"
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, "cot_inv_comparison.png")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CoT⁻¹ 比較グラフ生成（default / gainonly のみ）"
    )
    ...
    # Torque を除いた対象ベースライン
    target_baselines = ["default", "gainonly"]
    ...
    plot_cot_inv_results(all_results, args.output_dir)
