...
class Tegotae_Actor(nn.Module):
    ...
    def __init__(self, 
        ...
        self.reaction_net = ReactionNetMLP(input_dim=sensor_dim, output_dim=self.gain_out_dim)
        self.gain_net = GainNetMLP(sensor_dim=sensor_dim, output_dim=self.gain_out_dim)
    ...
    def forward(self, obs):
        ...
        reaction_mean = self.reaction_net(sensor_input)  # (batch, reaction_out_dim)
        gain_mean = self.gain_net(phi, sensor_input)  # (batch, gain_out_dim)

        # 最終的なアクション平均を結合
        full_action_mean = torch.cat([action_mean, reaction_mean, gain_mean], dim=1)  # (batch, action_dim)
...
class Tegotae_Policy(ActorCriticPolicy):
    ...
    def get_distribution_from_mean(self, mean):
        ...
        平均値から確率分布を作成するヘルパー
        """
        # log_std は ActorCriticPolicy が持っている学習可能パラメータ
        return DiagGaussianDistribution(mean.size(1)).proba_distribution(mean, self.log_std)