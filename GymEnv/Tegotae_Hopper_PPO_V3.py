...
class Tegotae_Hopper_PPO_v2_Env(gym.Env):
    ...
    def step(self, action):
        ...
        if self.disable_cpg_mod:
            ...
            if self.disable_reaction:
                net_reaction = self.grf / 1000 # 環境側で計算した地面反力をそのまま渡す（学習させない）
    ...
    def evaluate_robustness(self) -> dict:
        ...
        -------
        dict with keys:
            reward            : float        今ステップの報酬（報酬スキーム準拠）
            step_reward       : float        step毎の基本報酬
            height_reward     : float|None   着地時のみ: Gaussian高さ報酬
            efficiency_score  : float|None   着地時のみ: CoT⁻¹ (clip 0–10)
            jump_reward       : float|None   着地時のみ: height_reward + 0.5*efficiency_score相当
            terminated        : bool         終了条件を満たしたか
            com_z             : float        現在のCOM高さ [m]
            grf               : float        地面反力 [N]
            ...
            pose_error        : float        参照軌道との関節角誤差（CPG位相基準）
            accumulated_reward: float        サイクル内のstep_reward積算値
        """
        hip_act_id  = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, "hip_motor")
        knee_act_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, "knee_motor")
        if hip_act_id  == -1: hip_act_id  = 0
        ...
        # ---- CPG位相に基づく姿勢誤差 ----
        # __init__でキャッシュした参照軌道との関節角誤差（DirectTorque_Hopperと同一ロジック）
        phi = self.cpg.phi
        if isinstance(phi, torch.Tensor):
            phi = float(phi.detach().flatten()[0])
        else:
