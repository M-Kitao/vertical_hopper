...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        target_pos = [0, 0, 0.60 + 0.25 * math.cos(2 * math.pi * 1.0 * now)] 
        
        # B. 逆運動学 (IK) で目標角度を計算
        target_hip_angle, target_knee_angle = inverse_kinematics(target_pos[2])
        
        # C. モータへの指令         
        data.ctrl[hip_act_id] = (target_hip_angle)
        data.ctrl[knee_act_id] = (target_knee_angle)
        ...
        target_h_data.append(data.qpos[qpos_root_adr] - target_pos[2])  # 腰の高さ - 目標距離

        q_hip_data.append(data.qpos[model.jnt_qposadr[mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, "hip_joint")]])
        q_knee_data.append(data.qpos[model.jnt_qposadr[mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, "knee_joint")]])
