...
def main():
    ...
    # (簡易的な処置として、データの最初と最後を平均して上書きする等)
    mean_hip = (y_hip[0] + y_hip[-1]) / 2
    y_hip[0] = y_hip[-1] = mean_hip
    
    mean_knee = (y_knee[0] + y_knee[-1]) / 2
    y_knee[0] = y_knee[-1] = mean_knee

    # 3. 学習設定 (Least Squares)
    omega = 2 * np.pi  # ユーザー指定の角振動数
    basis_order = 100 # フーリエ級数の次数（高いほど複雑な波形にフィット）
    optimal_omega = find_optimal_omega(t_learn, y_hip, initial_omega=2*np.pi)
    print(f"最適化されたOmega: {optimal_omega}")

    # 計画行列 X の作成
    X = make_design_matrix(t_learn, optimal_omega, order=basis_order)
    ...
    # 通常の lstsq の代わりに、対角行列を足して解く
    lam = 0.1 # 正則化パラメータ（波打ちがひどい場合は大きくする）
    
    # w = (X.T @ X + lam * I)^-1 @ X.T @ y
    I = np.eye(X.shape[1])
    
    # Hip
    w_hip = np.linalg.inv(X.T @ X + lam * I) @ X.T @ y_hip
    
    # Knee
    w_knee = np.linalg.inv(X.T @ X + lam * I) @ X.T @ y_knee
    ...
    result_csv_path = 'CPG_orbit_fourier.csv'
