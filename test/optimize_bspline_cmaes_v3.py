...
def load_latest_csv(prefix='original_trajectory/omega_050'):
...
def plot_results(phi, dt,
    ...
    plt.tight_layout()
    plt.savefig('CPG_orbit_bspline_cmaes_v3_050.png', dpi=150, bbox_inches='tight')
    plt.show()
    print("図を保存しました: CPG_orbit_bspline_cmaes_v3_050.png")
...
def main():
    ...
    duration          = 1.54625 # 1周期の時間長 [s]（CSVデータから推定）
    start_time_offset = 0.1525     # データのどこから開始するか [s]（CSVの Time 列に基づく）
    ...
    out_path = 'CPG_orbit_bspline_cmaes_v3_050.csv'
